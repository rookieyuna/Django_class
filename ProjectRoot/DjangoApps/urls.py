...
urlpatterns = [
    ...
    path('', views.main, name = 'main'),#로켓화면 대신 메인화면 대체
    # http://127.0.0.1:8000/
    ...
    path('books/', include('books.urls')), #앱3 : 도서관 관리 앱
]

'''
    방법1의 경우 Url패턴이 변경되면 모든 항목을 수정해야 하므로 불편하다.
    하지만 방법2와 같이 파일을 2개로 나누게되면 하나의 패턴을 위한
    네임 스페이스가 생성되는 개념이므로 한번만 수정하면 되어 편하다.
